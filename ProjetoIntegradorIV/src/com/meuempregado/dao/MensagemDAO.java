...
public class MensagemDAO extends GenericDAO{
	...
	private String LISTAR_MENSAGEM = "select tb_mensagem.*, tb_resposta.idResposta, tb_resposta.description from tb_mensagem inner join tb_resposta on tb_mensagem.idresposta = tb_resposta.idResposta;";
	private String EXCLUIR = "DELETE FROM  tb_mensagem WHERE IDMENSAGEM=?;";
	private String ALTERAR = "UPDATE tb_mensagem SET nomeempresa= ?, email= ?, msg = ?,idresposta=?,endereco=?,vaga=?,horaentrevista=?,data=?,salario=?,beneficios=? WHERE idMensagem=?;";
		...
		if(rs != null) {
			while(rs.next()) {
				...
				Mensagem c = new Mensagem(rs.getInt("idMensagem"),rs.getString("nomeempresa"), rs.getString("email"), rs.getString("msg")
						,resposta,rs.getString("endereco"),rs.getString("vaga"),rs.getString("horaentrevista"),rs.getString("data"), rs.getFloat("salario"), rs.getString("beneficios"));
				lista.add(c);
			}
		}
